...
function Pokemon(props: IProps) {
    ...
    function changeToNextFlavorText(): void {
        setFlavorTextEntryIndex((oldIndex) =>
            pokemonSpecies &&
            oldIndex == pokemonSpecies.flavor_text_entries.length - 1
                ? 0
                : oldIndex + 1
        );
    }
    ...
    function changeToPreviousFlavorText(): void {
        setFlavorTextEntryIndex((oldIndex) =>
            pokemonSpecies && oldIndex === 0
                ? pokemonSpecies.flavor_text_entries.length - 1
                : oldIndex - 1
        );
    }
    ...
    useEffect(() => {
        async function makeRequest(): Promise<void> {
            ...
            data.flavor_text_entries = getFilteredFlavorTexts(
    ...
    useEffect(() => {
        if (pokemonSpecies) {
            setFlavorText(
                pokemonSpecies.flavor_text_entries[flavorTextEntryIndex]
                    .flavor_text
            );
        }
    ...
    return pokemonSpecies ? (
        <Section
            ...
            className={`${props.isFirst ? "highlight-2" : ""}`}
        >
                ...
                <div className="about">
                    ...
                    <span className="description">
                        <img
                            ...
                            src={downArrow}
                            alt="details icon"
                            onClick={changeToPreviousFlavorText}
                        />
                        <p className="flavor-text">{flavorText}</p>
                        <img
                            className="down-arrow right"
                            src={downArrow}
                            alt="details icon"
                            onClick={changeToNextFlavorText}
                        />
                    </span>
                </div>
                <img
